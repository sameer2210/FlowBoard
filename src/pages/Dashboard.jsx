...
import { BarChart3, Users, CheckCircle, Calendar, TrendingUp, Clock, AlertCircle } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const completedTasks = mockProjects.reduce((acc, project) => 
    acc + (project.tasks?.filter(task => task.completed === true).length || 0), 0
  );
  
  const averageProgress = totalProjects > 0 ? Math.round(
    mockProjects.reduce((acc, project) => acc + project.progress, 0) / totalProjects
  ) : 0;
              ...
              <div key={project.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-xs text-gray-500">Due: {formatDate(project.endDate)}</span>
                  </div>
                </div>
                <div className="ml-4">
...
//   const totalTasks = mockProjects.reduce((acc, project) => acc + (project.tasks?.length || 0), 0);
//   const completedTasks = mockProjects.reduce((acc, project) =>
//     acc + (project.tasks?.filter(task => task.completed).length || 0), 0
//   );
